...
int main() {
	...
	Random variable;
	Runway landing_runway(queue_limit), takingoff_runway(queue_limit), landing_runway_flex(queue_limit);
	for (int current_time = 0; current_time < end_time; ++current_time) {
		...
		switch (takingoff_runway.activity(current_time, moving_plane, takingoff)) {
		case land:
		...
		switch (landing_runway_flex.activity(current_time, moving_plane)){
			case land:
				moving_plane.land(current_time);
				break;
			case takingoff:
	...
	landing_runway_flex.shut_down(end_time);
	return 0;
}
